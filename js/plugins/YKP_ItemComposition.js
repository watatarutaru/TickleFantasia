 ...
 * 
 * 強化したときにステータス値を表示するには、強化するアイテムのメモ欄に
 * 　　<states:x:a,y:b...>と追加する必要があります。
 ...
 * 
 *  プラグインパラメータ「強化システムの設定」にある「表示パラメータ」で設定されたタグを読み取って表示させます。
 ...
 * 
 * ver 1.1.1 : 「強化」のみ利用時に表示カテゴリーを１つのみにした場合、エラーする問題を修正。
 * 
 * ver 1.1.0 : 表示カテゴリーを１つのみにした場合、エラーする問題を修正。
 * 
...
(() => {
    ...
    Window_ItemCompositionCommand.prototype.makeCommandList = function() {
        if (YukiKP.ItemComposition.isViewComposition()) {
            this.addCommand("料理する", "composition", this.isEnabledComposition());
        }
    ...
    Scene_ItemComposition.prototype.createCompositionMainWindow = function() {
        ...
        if (!this._compositionCategoryWindow.needsSelection()) {
            ...
            this._compositionNeedsWindow.y -= this._compositionCategoryWindow.height;
            this._compositionNeedsWindow.height += this._compositionCategoryWindow.height;
            this._compositionCategoryWindow.update();
            this._compositionCategoryWindow.hide();
            this._compositionCategoryWindow.deactivate();
            this.onCompositionCategoryOk();
        }
    };
    ...
    Scene_ItemComposition.prototype.createUpgradeMainWindow = function() {
        ...
        if (!this._upgradeCategoryWindow.needsSelection()) {
            ...
            this._upgradeNeedsWindow.y -= this._upgradeCategoryWindow.height;
            this._upgradeNeedsWindow.height += this._upgradeCategoryWindow.height;
            this._upgradeCategoryWindow.update();
            this._upgradeCategoryWindow.hide();
            this._upgradeCategoryWindow.deactivate();
            this.onUpgradeCategoryOk();
        }
    };
